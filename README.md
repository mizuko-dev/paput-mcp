...
- `paput_upsert_skill_sheet_projects` - Add or update one or more skill sheet projects in one call (pass a single project as a one-element array), including optional achievement bullets. Each project is processed independently.
...
- `paput_set_goals` - Sync the full goal list by ID: each item with an `id` updates that goal, items without an `id` are created, and existing goals whose IDs are absent from the list are deleted. Existing goal IDs are preserved. Destructive because it deletes goals left out of the list.

### Dashboard Analysis

- `paput_get_dashboard_analysis_context` - Get dashboard, goal, skill sheet, memo, note, and category context so the MCP client AI model can generate an analysis. The previously saved analysis is included as `saved_dashboard_analysis`.
...
- `paput_mark_processed_sessions` - Mark one or more reviewed sessions as processed in one call when no candidates are added.
- `paput_list_pending_candidates` - List pending candidates.
- `paput_update_pending_candidates` - Update the fields of one or more pending candidates in one call before they are saved (partial update, pending only).
- `paput_save_pending_candidates` - Save one or more approved pending candidates as PaPut memos in one call.
- `paput_discard_pending_candidates` - Discard one or more pending candidates in one call.
...
- `paput_save_pending_candidates` requires explicit user approval to save pending candidates to PaPut.
...
- `paput_set_goals` syncs the full goal list and deletes existing goals whose IDs are left out of the list, so it should be used only when the desired final goal set is known; requires user confirmation before deleting goals.
...
- `paput_discard_pending_candidates` removes pending candidates from the save flow and should be confirmed when a candidate may still be useful.
...
4. Use `paput_mark_processed_sessions` when reviewed sessions have no candidates.
...
- **Still stuck**: Contact `info@paput.io` or open an issue at https://github.com/mizulba-dev/paput/issues.
