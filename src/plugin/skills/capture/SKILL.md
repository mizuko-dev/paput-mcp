...
4. For each candidate, check semantically similar existing memos in two stages. First call `paput_search_memo` with the candidate title or a one-line gist as the query and use the compact index results to identify the strongest matches. Then call `paput_search_memo` again with `ids` containing only those top result IDs to fetch their bodies, and compare those bodies against the candidate body. Treat results with a score around 0.85 or higher as near-duplicates, and results around 0.7 to 0.85 as overlap that needs body comparison. Only results with a `score` are semantic near-matches; results without one are keyword-only hits, which already cover the case where the candidate centers on an exact name or identifier.
