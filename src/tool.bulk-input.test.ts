...
function installFetch(
  routes: Array<{ match: (url: string, method: string) => boolean; payload: unknown }>,
): { calls: FetchCall[] } {
  ...
  const fetchMock = vi.fn(async (url: string, options: RequestInit = {}) => {
    ...
    if (!route) {
      return new Response(JSON.stringify({ error: `no route for ${method} ${url}` }), {
        status: 500,
      });
    }
...
function captureCallTool() {
  ...
  const fakeServer = {
    setRequestHandler: (schema: unknown, fn: (request: unknown) => Promise<Record<string, unknown>>) => {
...
describe('bulk tools tolerate mixed input through setupTool', () => {
    ...
    const { calls } = installFetch([
      {
        ...
        payload: {
          ...
          marked_count: 1,
          failed_count: 0,
          results: [{ index: 0, session_id: 'a', status: 'marked', error: null }],
        },
      },
    ]);
  ...
  it('upsert_skill_sheet_projects: invalid element does not block the valid one', async () => {
    const { calls } = installFetch([
      {
        ...
        payload: {
          ...
          updated_count: 0,
          failed_count: 0,
          results: [{ index: 0, id: 10, title: 'PaPut', action: 'created', error: null }],
        },
      },
    ]);
    const callTool = captureCallTool();
    ...
    const apiCalls = calls.filter((c) => c.method === 'POST');
    expect(apiCalls).toHaveLength(1);
    expect((apiCalls[0].body as { projects: unknown[] }).projects).toHaveLength(1);
    expect(result.structuredContent).toMatchObject({
      created_count: 1,
      failed_count: 1,
    });
  ...
  it('save_pending_candidates: invalid element does not block the valid one', async () => {
    const { calls } = installFetch([
      ...
      {
        match: (url, method) => method === 'POST' && url.endsWith('/api/v1/mcp/memos'),
        payload: {
          success: true,
          created_count: 1,
          failed_count: 0,
      ...
      {
        match: (url, method) =>
          method === 'PUT' && url.endsWith('/api/v1/mcp/knowledge-candidates/save'),
        payload: {
          success: true,
          saved_count: 1,
          failed_count: 0,
          results: [
            { index: 0, candidate_id: 'c1', saved_memo_id: 100, status: 'saved', error: null },
          ],
        },
      },
    ]);
    ...
    expect((putCalls[0].body as { candidates: unknown[] }).candidates).toHaveLength(1);
    expect(result.structuredContent).toMatchObject({
      saved_count: 1,
      failed_count: 1,
    });
