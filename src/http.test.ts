...
describe('HTTP MCP server security handling', () => {
  ...
  it('completes the handshake with a project alias header without calling the API', async () => {
  ...
  it('passes the project alias header through to the MCP server factory', async () => {
    ...
    expect(response.status).toBe(200);
    expect(body).toContain('"serverInfo"');
    // query と無関係にヘッダー値が factory まで届いていること。
  ...
  it('rejects a malformed project alias header with a validation error', async () => {
    const port = await startTestServer();
  ...
  it.each([
    ['project_alias', 'project_alias=paput'],
    ['alias', 'alias=paput'],
    ['malformed project_alias', 'project_alias=Bad%20Alias'],
  ])('ignores the %s query parameter', async (_label, query) => {
    serverFactoryCall.mockClear();
    const port = await startTestServer();
    ...
    expect(body).toContain('"serverInfo"');
    const factoryOptions = serverFactoryCall.mock.calls.at(-1)?.[0];
    expect(factoryOptions?.projectAlias).toBeUndefined();
    expect(factoryOptions?.resolveProject).toBeUndefined();
  });
...
describe('resolveProjectAlias', () => {
  it('reads the alias from the header', () => {
    expect(resolveProjectAlias('gaikodb')).toBe('gaikodb');
  });

  it('returns no alias when the header is absent', () => {
    expect(resolveProjectAlias(null)).toBeNull();
    expect(resolveProjectAlias(undefined)).toBeNull();
  });

  it('treats an unexpanded placeholder header as no alias', () => {
    expect(resolveProjectAlias('${user_config.project_alias}')).toBeNull();
  });

  it('rejects a malformed header so the caller can return 400', () => {
    expect(resolveProjectAlias('Bad Alias')).toBe(false);
    expect(resolveProjectAlias('ab')).toBe(false);
  });
});
